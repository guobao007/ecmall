    ...
    this.init = function()
    {
        if (this.specQty >= 1)
        {
            ...
            for (var i = 0; i < spec1Values.length; i++)
            {
                ...
                {
                    $(".nc-spec ul:eq(0)").append('<li class="sp-txt"><a href="javascript:void(0)" class="hovered" onclick="selectSpec(1, \'' + spec1Values[i] + '\', this)">' + spec1Values[i] + '<i></i></a></li>');
                }
                else
                {
                    $(".nc-spec ul:eq(0)").append('<li class="sp-txt"><a href="javascript:void(0)" onclick="selectSpec(1, \'' + spec1Values[i] + '\', this)">' + spec1Values[i] + '<i></i></a></li>');
                }
            }
        }
        if (this.specQty >= 2)
        {
            ...
            for (var i = 0; i < spec2Values.length; i++)
            {
                ...
                {
                    $(".nc-spec ul:eq(1)").append('<li class="sp-txt"><a href="javascript:void(0)" class="hovered" onclick="selectSpec(2, \'' + spec2Values[i] + '\', this)">' + spec2Values[i] + '<i></i></a></li>');
                }
                else
                {
                    $(".nc-spec ul:eq(1)").append('<li class="sp-txt"><a href="javascript:void(0)" onclick="selectSpec(2, \'' + spec2Values[i] + '\', this)">' + spec2Values[i] + '<i></i></a></li>');
                }
            }
        }
...
/* 选中某规格 num=1,2 */
function selectSpec(num, spec, liObj)
{
    goodsspec['spec' + num] = spec;
    $(liObj).parent().parent().children().children().removeClass("hovered");
    $(liObj).attr("class", "hovered");
