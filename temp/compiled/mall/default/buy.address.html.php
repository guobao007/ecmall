<div class="ncc-receipt-info">
    ...
    <div id="addr_list" class="ncc-candidate-items">
        <ul>
            <li>
                <span class="true-name"><?php echo $this->_var['address_info']['consignee']; ?></span>
                <span class="address"><?php echo $this->_var['address_info']['region_name']; ?><?php echo $this->_var['address_info']['address']; ?></span>
                <span class="phone"><i class="icon-mobile-phone"></i><?php if ($this->_var['address_info']['phone_tel']): ?><?php echo $this->_var['address_info']['phone_tel']; ?><?php else: ?><?php echo $this->_var['address_info']['phone_mob']; ?><?php endif; ?></span>
            </li>
        </ul>
    </div>
</div>
...
$('#edit_reciver').on('click',function(){
    ...
    $('#addr_list').load(SITE_URL+'/index.php?app=buy&act=load_addr');
});
//异步显示每个店铺运费 city_id计算运费area_id计算是否支持货到付款
function showShippingPrice(city_id,area_id) {
	$('#buy_city_id').val('');
    $.post(SITE_URL + '/index.php?act=buy&op=change_addr', {'freight_hash':'<?php echo $output['freight_hash'];?>',city_id:city_id,'area_id':area_id}, function(data){
