...
function drop_cart_item(cart_id){
    ...
    $.getJSON('index.php?app=cart&act=drop&rec_id=' + cart_id, function(result){
        if(result.done){
            //删除成功
            if(result.retval.cart.quantity == 0){//判断购物车是否为空
 ...
 * @param input
 */
function change_quantity(store_id, cart_id, spec_id, input){
    var subtotal = $('#item' + cart_id + '_subtotal');
    //暂存为局部变量，否则如果用户输入过快有可能造成前后值不一致的问题
    var _value = input.value;
    $.getJSON('index.php?app=cart&act=update&spec_id=' + spec_id + '&quantity='  + _value, function(result){
    	$(input).attr('changed', _value);
    	if(result.done){
            $('#item' + cart_id + '_price').html(number_format(result.retval.goods_price,2));
            subtotal.html(number_format(result.retval.subtotal,2));
            $('#cart_id'+cart_id).val(cart_id+'|'+_value);
        }else{
            if(result.retval.state == 'invalid'){
                subtotal.html(0.00);
            ...
            if(result.retval.state == 'shortage'){
                $('#item' + cart_id + '_price').html(number_format(result.retval.goods_price,2));
                $('#cart_id'+cart_id).val(cart_id+'|'+result.retval.goods_num);
                $(input).val(result.retval.goods_num);
                alert(result.msg);
                return;
            }
            //更新失败
